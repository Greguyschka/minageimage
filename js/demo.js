 ...
 */
{
	...
	class Grid {
		constructor(el, options) {
			...
			this.DOM = {};
			this.DOM.el = el;
			this.DOM.items = Array.from(this.DOM.el.querySelectorAll('li.grid__item:not(.grid__item--name):not(.grid__item--title):not(.grid__item--text)'));
		...
		show(filled = false, animation = true) {
			return new Promise((resolve, reject) => {
				...
				this.DOM.el.classList.remove('wb-inv');
				const promises = [];
		...
		hide(filled = false, animation = true) {
			return new Promise((resolve, reject) => {
				...
				Promise.all(promises).then(() => {
					...
					this.DOM.el.classList.add('wb-inv');
					resolve();
				});
			});
		}
